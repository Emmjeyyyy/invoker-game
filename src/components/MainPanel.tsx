...
export const MainPanel: React.FC<MainPanelProps> = ({ onOpenSettings }) => {
  const { isStarted, startGame, endGame, targetSpell, currentOrbs, slotD, slotF, keybinds } = useGameStore();
    ...
    <div className="flex flex-col flex-1 min-h-0 justify-center bg-panel border border-panelBorder rounded-xl p-4 sm:p-8 shadow-2xl items-center relative w-full overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-col items-center justify-center w-full mb-6 sm:mb-12 mt-8 sm:mt-0">
        <div className="flex flex-col items-center w-full justify-center">
          ...
          >
            <div className="w-24 h-24 sm:w-32 sm:h-32 lg:w-48 lg:h-48 2xl:w-64 2xl:h-64 mx-auto bg-black/50 border border-panelBorder rounded mb-2 sm:mb-6 2xl:mb-8 shadow-2xl flex items-center justify-center overflow-hidden">
            ...
            <h2 className="text-2xl sm:text-4xl lg:text-5xl 2xl:text-6xl text-white tracking-wider">{targetSpell?.name}</h2>
          </motion.div>
        </div>
      </div>
      ...
      {!isStarted && (
        <button
          ...
          className="mt-8 2xl:mt-12 px-8 py-4 lg:px-12 lg:py-6 border border-panelBorder rounded-xl hover:bg-panelBorder/30 transition-all group flex flex-col items-center"
        >
          <span className="text-xl lg:text-3xl 2xl:text-4xl tracking-wider group-hover:text-white transition-colors">Start Game</span>
        </button>
      )}
    </div>
  );
